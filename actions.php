...
switch(htmlspecialchars(sanitize_text_field($_POST['pdckl_hidden'])))
{
    ...
    case 'order_add':
        ...
        $pdckl_flink = '<a href="' . sanitize_text_field($_POST['pdckl_link']) . '">' . sanitize_text_field($_POST['pdckl_link_name']) . '</a> - ' . sanitize_text_field($_POST['pdckl_link_description']);
        ...
        $link = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix . "pdckl_links ORDER BY id DESC LIMIT 1", ARRAY_A);
        _e('<div class="updated"><p><strong>' . sprintf($pdckl_lang['n_orders_added'], $link['id']) . '</strong></p></div>');
    break;
    
    case 'order_edit':
        $id_order = (int) $_POST['pdckl_id_order'];
    ...
    case 'orders_backup_upload':
        if($_FILES['backupfile']['name'] != '')
        {
            if(pathinfo($_FILES['backupfile']['name'], PATHINFO_EXTENSION) == 'xml')
            {
                if(move_uploaded_file($_FILES['backupfile']['tmp_name'], PDCKL_PLUGIN_DIR . 'orders_backup.xml'))
                {
                    $xml = @simplexml_load_file(PDCKL_PLUGIN_DIR . 'orders_backup.xml');
                    if($xml != false && count($xml->order) != 0)
                    {
                        $wpdb->query("TRUNCATE TABLE " . $wpdb->prefix . "pdckl_links");
                        foreach($xml->order as $order)
                        {
                            $wpdb->insert($wpdb->prefix . "pdckl_links", array(
                                'id'      => $order->IDOrder,
                                'id_post' => $order->IDPost,
                                'time'    => $order->Date,
                                'link'    => $order->Link,
                                'active'  => $order->Active
                            ));
                        }
                        _e('<div class="updated"><p><strong>' . $pdckl_lang['n_orders_backup_loaded'] . '</strong></p></div>');
                    }
                    else
                        _e('<div class="error"><p><strong>' . $pdckl_lang['n_orders_backup_up_error'][3] . '</strong></p></div>');
                }
                else
                    _e('<div class="error"><p><strong>' . $pdckl_lang['n_orders_backup_up_error'][2] . '</strong></p></div>');
            }
            else
                _e('<div class="error"><p><strong>' . $pdckl_lang['n_orders_backup_up_error'][1] . '</strong></p></div>');
        }
        else
            _e('<div class="error"><p><strong>' . $pdckl_lang['n_orders_backup_up_error'][0] . '</strong></p></div>');
    break;
    ...
    case 'css_save':
        if(!check_admin_referer( 'save-css' )) {
          ...
          file_put_contents(PDCKL_PLUGIN_DIR . 'styles/podclankova-inzerce.css', $_POST['pdckl_css']);
          if(file_get_contents(plugins_url('styles/podclankova-inzerce.css', __FILE__)) == $_POST['pdckl_css'])
          {
    ...
    case 'css_reset':
        if(!check_admin_referer( 'reset-css' )) {
          ...
          $css_backup = file_get_contents(plugins_url('styles/podclankova-inzerce_original.css', __FILE__));
          file_put_contents(PDCKL_PLUGIN_DIR . 'styles/podclankova-inzerce.css', $css_backup);
          if(file_get_contents(plugins_url('styles/podclankova-inzerce_original.css', __FILE__)) == file_get_contents(plugins_url('styles/podclankova-inzerce.css', __FILE__)))
          {
...
{ 
    ...
    case 'backup_restore':
        _e('<div class="updated">
                <p><strong>' . $pdckl_lang['n_orders_backup_restore'] . '</strong></p>
                <p>
                    <form enctype="multipart/form-data" action="' . PDCKL_ADMIN_LINK . '&s=orders" method="POST">
                        <input style="margin-right:5px;" name="backupfile" type="file" />  
                        <input type="hidden" name="pdckl_hidden" value="orders_backup_upload">
                        <input type="submit" class="button-primary" value="' . $pdckl_lang['btn_restore_backup'] . '" />
                    </form>
                </p>
            </div>');
    break;
