...
{?>
    <link rel="stylesheet" type="text/css" href="<?php _e(plugins_url('styles/podclankova-inzerce_admin.css', __FILE__)); ?>">
    <link rel="stylesheet" type="text/css" href="<?php _e(plugins_url('js/vTip/css/vtip.css', __FILE__)); ?>">
    <script type="text/javascript" src="<?php _e(plugins_url('webdeal_lib/WDConnect.js', __FILE__)); ?>"></script>
    <script type="text/javascript" src="<?php _e(plugins_url('js/vTip/vtip.js', __FILE__)); ?>"></script>
<?php
wp_enqueue_script('jquery');
 _e('
...
{
    global $pdckl_lang;
    return '<img src="' . plugins_url('img/icon_help.png', __FILE__) . '" title="' . $pdckl_lang[$hid] . '" class="vtip" />';
}
...
function pdckl_footer()
{
    ...
    wp_enqueue_script('wdconnect', plugins_url('webdeal_lib/WDConnect.js', __FILE__));
}
...
function pdckl_clearcache($post_id) {
  ...
  if(in_array('zencache', $plugin_array)) {
    ...
    $url = str_replace('/plugins/podclankova-inzerce', '', dirname(__FILE__)).'/cache/comet-cache/cache/http/'.str_replace('.', '-', str_replace('https://', '', str_replace('http://', '', get_option('siteurl')))).'/'.$yy.'/'.$mm.'/'.$dd.'/';
    if(!is_dir($url)) {
      $url = str_replace('/plugins/podclankova-inzerce', '', dirname(__FILE__)).'/cache/comet-cache/cache/http/'.str_replace('.', '-', str_replace('https://', '', str_replace('http://', '', get_option('siteurl')))).'/'.$yy.'/'.$mm.'/*';
    } else {
      $url .= '/*';
    }
    ...
    switch($wp_query->query_vars['pdckl']) {
      ...
      case 'checkout':
        ...
        if($pdckl_gateway_link != "" and $pdckl_gateway_link_name != "" and $pdckl_gateway_desc != "")
        {
            ...
            {
                $return_url = explode("?", "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
                $request = array('token' => get_option('pdckl_wd_token'),
                ...
                $ch = curl_init('https://www.api.copywriting.cz/podclankova-inzerce/v2/checkout.php?mode=data');
                ...
                {
                    header("Location: https://www.api.copywriting.cz/podclankova-inzerce/v2/checkout.php?mode=login&token_order=" . $json->token_order);
                }
                else {
                  _e ('
                      <html>
              ...
              header("Location: https://www.api.copywriting.cz/podclankova-inzerce/v2/card.php?mode=overview&link=" . base64_decode($pdckl_gateway_flink) . "&post=" . base64_decode($_POST['url_post']) . "&price=" . $price);
              exit;
            } else {
        ...
        if(isset($json->post_id) && isset($json->link) && isset($json->token))
        {
            if($json->token == get_option('pdckl_wd_token'))
            {
                ...
                if($inserted) {
                  ...
                  echo json_encode(array('status'  =>  'non_inserted'));
                  exit;
                }
            } else {
              echo json_encode(array('status'  =>  'bad_token'));
              exit;
            }
        } else {
          echo json_encode(array('status'  =>  'mising_parameters'));
          exit;
        }
      break;
      ...
      case 'ping_info':
        $response = array(
          'pdckl_active' =>  get_option('pdckl_active'),
        );
